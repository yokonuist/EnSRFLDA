...
!  tem1=smwsw*wwsn+smunw*(1-wwsn)
!  tem2=smwse*wwsn+smune*(1-wwsn)
!  tem3=tem1*wsew+tem2*(1-wsew)           !  for lower soilm

!  tem1=smlsw*wwsn+smlnw*(1-wwsn)
!  tem2=smlse*wwsn+smlne*(1-wwsn)
!  tem4=tem1*wsew+tem2*(1-wsew)           !  for upper soilm
...
! if( abs(innovationmean) > 0.8 ) goto 200   
  if( abs(innovationmean) > 5*obs_err_smois ) goto 200 !3*obs_error
  open(888,file='./diag_obs_smois.txt')
  write(888,*)temp_soil_m(iobs),temp_latsnd(iobs),temp_lonsnd(iobs),iobs
...
!  hx(i)=tem3*(1-wul)+tem4*wul
  ...
  tem1=stusw*wwsn+stunw*(1-wwsn)
  tem2=stuse*wwsn+stune*(1-wwsn)
  tem3=tem1*wsew+tem2*(1-wsew)           !  for upper soilt

  tem1=stlsw*wwsn+stlnw*(1-wwsn)
  tem2=stlse*wwsn+stlne*(1-wwsn)
  tem4=tem1*wsew+tem2*(1-wsew)           !  for lower soilt
...
!  if( abs(innovationmean) > 10 ) goto 205 
 if( abs(innovationmean) > 2*obs_err_tslb ) goto 205 !3*obs_error 
  open(889,file='./diag_obs_tslb.txt')
  write(889,*)temp_soil_t(iobs),temp_latsnd(iobs),temp_lonsnd(iobs),iobs
...
!print*,'istatnum1,tunw',istatnum1,tunw
 ...
 ! hx(i)=tem1*(1-wsew)+tem2*wsew           !  for surface temp
  ...
  tem1=strsw*wwsn+strnw*(1-wwsn)
  tem2=strse*wwsn+strne*(1-wwsn)
  hx(i)=tem1*wsew+tem2*(1-wsew) 
...
!  stop
  ...
  open(900,file='./diag_obs_tsk.txt')
  write(900,*)temp_soil_st(iobs),temp_latsnd(iobs),temp_lonsnd(iobs),iobs
