...
include 'namelist.inc'
  ...
  integer               :: analysis_var_num,ra_data_num,numstat,iradar
  integer               :: iobs,ivar
  integer               :: nn
  ...
  rv_grid = miss_data
open(1101,file='./radar_angle.dat')
do i=1,lev_num
 read(1101,'(F5.2)') evl(i)
enddo
close(1101)
...
!*********************for mean radial velocity  hx cal***********************   
  ...
  !-----------------------------------
  call cal_lv_num(evlr,evl,ilv,lev_num)
  ...
  do i=1,ens_num
  spd(i)=spd(i) + (hx(i)-hxmean)**2
  enddo
  
  rmse_rv = rmse_rv + innovationmean**2
  temp1=temp1+1  
  endif          ! for valid radial velocity  
  
  enddo          ! do iobs=1,ra_data_num
  ...
  open(1100,file='./rv_grid_'//lab_radar_num//'_'//time_lab//'',form='binary')
    write(1100) rv_grid  
  close(1100)
...
!*********************for pert reflectivity  hx cal*********************** 
  ...
  rmse_rf = rmse_rf + innovationmean**2
  temp1=temp1+1  
  ...
  open(1100,file='./rf_grid_'//lab_radar_num//'_'//time_lab//'',form='binary')
    write(1100) rf_grid  
  close(1100)  
...
subroutine cal_lv_num(evlr,evl,ilv,lev_num)
...
do i=1,lev_num
	...
	temr01=evlr*180/3.1415926

  if(abs(temr01-evl(i))<0.1) then
		ilv=i
  endif

enddo

end subroutine cal_lv_num